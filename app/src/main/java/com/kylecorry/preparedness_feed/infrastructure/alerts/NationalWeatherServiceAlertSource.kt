...
class NationalWeatherServiceAlertSource(context: Context, private val area: String) : AtomAlertSource(context) {
    ...
    override fun postProcessAlerts(alerts: List<Alert>): List<Alert> {
        return alerts.map {
            it.copy(source = "National Weather Service", type = "NWS", useLinkForSummary = false)
        }
    }
}